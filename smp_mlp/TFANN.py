...
class CNNR(ANNR):
    ...
    def CreateModel(self):
        ...
        self.O, self.W, self.B = _CreateCNN(self.AF, self.pad, self.ws,
                                            self.X)  # Create graph; YH is output from feedforward
        if self.Y is None:
...
class DCNN(ANNR):
    ...
    def CreateModel(self):
        ...
        if self.reg is not None:  # Regularization can prevent over-fitting
            self.loss += _CreateL2Reg(self.W, self.B) * self.reg
        self.optmzr = _GetOptimizer(self.optmzrn, self.lr).minimize(
            self.loss)  # Get optimizer method to minimize the loss function
